  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-6 px-6 py-10">
      <div className="flex flex-col gap-1">
        ...
        <h1 className="text-xl font-semibold text-black dark:text-zinc-50">Cost / session history</h1>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
        ...
        <p className="text-xs text-zinc-500">
          Drives the same energy math as issue #9&apos;s local charging simulation, without requiring a live CSMS
          connection — start a session on a connector, then stop it to see the Cost row and post-charge popup.
        </p>
        {connectors === null ? (
                ...
                {connector.activeSession ? (
                  <>
                    <span className="text-xs text-zinc-500">
                      card {connector.activeSession.idTag} · {(connector.activeSession.currentEnergyWh / 1000).toFixed(3)}{" "}
                      kWh so far
                    </span>
                    <select
